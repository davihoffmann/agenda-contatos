...
class ContactHelper {
  ...
  Future<Database> initDb() async {
    ...
    return await openDatabase(path, version: 1, onCreate: (Database db, int newerVersion) async {
      await db.execute(
        "CREATE $contactTable ($idColumn INTEGER PRIMARY KEY, $nameColumn TEXT, $emailColumn TEXT, $phoneColumn TEXT, "
        "$imgColumn TEXT);"
      );
    });
  }
  ...
  Future<List<Contact>> getAllContacts() async {
    ...
    List<Contact> listContact = listMap.map<Contact>((map) => Contact.fromMap(map)).toList();
    return listContact;
  }
